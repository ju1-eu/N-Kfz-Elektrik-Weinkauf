...
while [ "$JANEIN" -eq 1 ]; do
	...
	printf "\n  0) Projekt aufräumen"
	printf "\n  1) Projekt erstellen"
	printf "\n  2) Markdown in (LaTeX) + sed (Suchen/Ersetzen) > LaTeX-Syntax + Kopie > tex/ + Files löschen"
	...
	###########################################################################
		...
		# FEHLER unter mac!!!!
	    ./$SCRIPTE/sed.sh
		# Kopie (*.tex) nach tex/ + Files löschen (*.tex-e)
		echo "+ Kopie (*.tex) nach tex/ + Files löschen (*.tex-e)"
		mv ./$TEXPANDOC/*.tex ./$CONTENT/tex
		rm -rf ./$TEXPANDOC/*.tex-e
